...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth]{../../figures/en/three-layer-concept-map.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{../../figures/en/sect-sutra-semantic-map.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{../../figures/en/sect-sutra-chunk-distribution-overview.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{../../figures/en/sect-sutra-chunk-distribution-focus.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{../../figures/en/sect-sutra-chunk-overlap-heatmap.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{../../figures/en/sect-sutra-overlap-vs-centroid.png}
...
\centering
\small
\caption{Bridge Chunks for Representative Pairs. KGS denotes Kyogyoshinsho.}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth]{../../figures/en/sect-sutra-similarity-heatmap.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\linewidth]{../../figures/en/amida-three-layer-difference.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\linewidth]{../../figures/en/amida-two-translation-comparison.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.86\linewidth]{../../figures/en/shinran-amida-source-markers.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{../../figures/en/shinran-kyogyoshinsho-amida-chunk-affinity.png}
  \caption{Maximum similarity from Kyogyoshinsho chunks to the two Amitabha translations. Red indicates proximity to Xuanzang, blue to Kumarajiva.}
...
\begin{table}[H]
\centering
\caption{Mean Weights in the Kyogyoshinsho Three-Layer Source-Mixture Map}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{../../figures/en/kyogyoshinsho-three-layer-source-mixture.png}
  \caption{Three-layer source-mixture map for Kyogyoshinsho. The upper panel is the semantic layer based on embedding similarity, the middle panel the stylistic/lexical layer based on character n-gram TF-IDF, and the lower panel the source-marker layer based on scripture-title, translator-name, and fixed-phrase dictionaries. Weights are relative within each layer. In the semantic and stylistic/lexical layers, all weight is necessarily distributed among the four reference sources; the largest weight is not therefore a true source attribution.}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{../../figures/en/kyogyoshinsho-volume-source-means.png}
  \caption{Volume-level source tendencies in Kyogyoshinsho. Weights in the semantic, stylistic/lexical, and source-marker layers are averaged over chunks assigned to each volume.}
...
\centering
\small
\caption{Dominant Source by Volume-Level Mean in Kyogyoshinsho}
...
\noindent\textbf{Top-1 accuracy, MRR, and ROC-AUC:} Retrieval-evaluation metrics. Top-1 accuracy asks whether the correct item is ranked first. MRR measures how high the first correct item appears on average. ROC-AUC summarizes how well positive and negative pairs are separated across thresholds.

\noindent\textbf{Parallel passages, intertextuality, and cross-linguistic semantic textual similarity:} Parallel-passage detection looks for overlapping or corresponding passages. Intertextuality refers more broadly to textual relations such as quotation, allusion, reuse, and reference. Cross-linguistic semantic textual similarity compares meaning across languages.

\noindent\textbf{Manifest, JSON, and HTML:} A manifest is a structured list of target texts, metadata, and source information. JSON is a machine-readable data format used by the scripts and viewer. HTML is the document format used for the web paper, process report, and static viewer.

\noindent\textbf{OpenAI API, API key, and SDK:} The OpenAI API is the service interface used here to generate embeddings. An API key authorizes access to that service and must not be committed to git. The SDK is the Python library used by local scripts to call the API.
...
The text and figures of this paper are released under Creative Commons Attribution 4.0 International (CC BY 4.0), \url{https://creativecommons.org/licenses/by/4.0/}. The accompanying analysis code is licensed under the MIT License, as stated in \texttt{LICENSE-CODE} in the repository. Source texts obtained from SAT, Seikyo DB, 84000, and other providers are not redistributed in the paper or public data and remain subject to each provider's terms of use.
