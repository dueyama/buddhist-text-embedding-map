...
\hypersetup{
  ...
  urlcolor=blue,
  citecolor=blue,
  pdftitle={意味・文体・典拠マーカーの三層地図による仏教文献探索：宗派別参照群・阿弥陀経二訳・親鸞文献の予備的分析},
  pdfauthor={上山大信},
  pdfkeywords={漢訳仏典, 仏教文献情報学, 意味埋め込み, 三層地図, 典拠マーカー, 阿弥陀経, 異訳比較, 親鸞, 教行信証, 多言語比較}
}
...
\title{意味・文体・典拠マーカーの三層地図による仏教文献探索：\\[0.35em]宗派別参照群・阿弥陀経二訳・親鸞文献の予備的分析}
\author{上山大信}
\date{2026年6月2日}
...
\begin{abstract}
本稿は、漢訳仏典および日本撰述仏教文献を対象に、文献間の近接を意味層、文体・語彙層、典拠マーカー層の三層に分けて可視化する探索的枠組みを提案する予備的研究である。対象には、日本仏教の宗派別参照テキスト群を中心とする15テキスト、433チャンクを用いた。各本文を \texttt{text-embedding-3-large} で埋め込み、文字n-gram TF-IDF による文体・語彙層、および経名・訳者名・固定句などの明示マーカーに基づく典拠マーカー層を重ねた。

結果として、阿弥陀経二訳は意味層では近く、文体・語彙層では距離を示した。また、『教行信証』に対する三層参照源混合地図では、意味層、文体・語彙層、典拠マーカー層が異なる参照源混合を示し、意味的に近いこと、文体・語彙的に近いこと、典拠・学習経路として近いことは同じではないことが確認された。多言語パイロットでは、Toh 106 と T0676 の組み合わせに限り、84000英訳の最近傍が対応漢訳となった。ただし、本稿は小規模コーパスと小規模辞書による予備研究であり、典拠マーカー層は引用・参照関係そのものの完全検出ではない。
\end{abstract}

\noindent\textbf{キーワード：} 漢訳仏典，仏教文献情報学，意味埋め込み，三層地図，典拠マーカー，阿弥陀経，異訳比較，親鸞，教行信証，多言語比較
...
近年の大規模言語モデルおよび埋め込みモデルは、テキストの意味的近さを高次元ベクトルとして表現できる。本研究では、この手法を仏教文献に適用し、宗派ごとの参照テキスト群の配置を可視化できるかを検討する。あわせて、阿弥陀経の異訳や同一訳者の複数テキストを用い、意味的近さと翻訳文体の差がどのように現れるかを予備的に確認する。さらに、親鸞文献を対象に、意味的に近いこと、文体的に近いこと、典拠・学習経路として近いことが同じではない点をチャンク単位で可視化する。漢訳仏典と84000 Reading Roomで公開されている英訳も対照し、意味埋め込みが言語を越えた同一経典性をどの程度拾えるかを試験的に確認する。
...
\begin{enumerate}
  ...
  \item 『教行信証』の各チャンクは、意味層、文体・語彙層、典拠マーカー層で、浄土三部経および阿弥陀経二訳へどのように異なる近さを示すか。
...
\begin{longtable}{@{}p{0.10\linewidth}p{0.22\linewidth}p{0.14\linewidth}p{0.12\linewidth}p{0.12\linewidth}p{0.21\linewidth}@{}}
...
本稿では、阿弥陀経二訳および親鸞文献を読むための試験的な枠組みとして、意味層、文体・語彙層、典拠マーカー層の三層を分ける（図\ref{fig:three-layer-concept}）。この三層を同じ対象チャンク列に重ねて表示する図を、本稿では三層参照源混合地図（source-mixture map）と呼ぶ。中心命題は、意味的に近いこと、文体・語彙的に近いこと、典拠・学習経路として近いことは同じではない、という点である。

チャンク $a_i$ とチャンク $b_j$ の間に、三種類のスコアを定義する。意味スコア $S_{ij}$ は、\texttt{text-embedding-3-large} によるチャンク埋め込みのコサイン類似度である。文体・語彙スコア $T_{ij}$ は、2から5文字の文字n-gram TF-IDF によるコサイン類似度である。ここでTF-IDF は、『教行信証』チャンクと四参照源チャンクを合わせた集合に fit した。したがって、本稿の文体・語彙層は厳密な stylometry ではなく、語彙・表記的近接を捉えるための初期的な操作化である。典拠マーカースコア $C_i(s)$ は、経名、訳者名、固定句、阿弥陀経二訳に特徴的な語句、浄土三部経に関わる語句を辞書として用い、対象チャンク内のマーカー出現を参照源 $s$ ごとに数えた予備的指標である。本稿でいう典拠マーカー層は、引用・参照関係そのものの完全検出ではなく、経名・訳者名・固定句などの明示的マーカーに基づく探索的 proxy である。
...
意味層と文体・語彙層の参照源重みは、softmax によって四参照源内の相対重みに変換した。意味層では $\tau_S=0.04$、文体・語彙層では $\tau_T=0.025$ を用いた。数値安定化のため、実装では各行の最大値を差し引いてから指数化したが、スコアの標準化やz-score化は行っていない。これらの温度パラメータは、参照源間の相対差を可視化しやすくするための予備的設定であり、現段階では独立した検証データによって最適化したものではない。
...
典拠マーカー層では、辞書マーカーの出現数を参照源別に正規化し、マーカーが検出されないチャンクは「未検出」とした。1チャンク内に複数参照源のマーカーが出た場合は、出現数に比例して重みを分けた。意味層と文体・語彙層は7チャンク幅の移動平均、典拠マーカー層は9チャンク幅の移動平均で表示した。各層の重みは、同一層内で参照源間の相対的な近さを見るための値であり、層を越えて数値を直接比較するものではない。また、意味層と文体・語彙層では四参照源内で必ず重みが配分されるため、四参照源以外に由来するチャンクにも「その他」カテゴリはまだ導入していない。

\begin{table}[H]
\centering
\caption{典拠マーカー辞書の代表例}
...
玄奘訳称讃浄土経 & 稱讃淨土／稱讚淨土、稱讃淨土經／稱讚淨土經、玄奘、舍利子、恒河沙、慈悲加祐、攝受法門、百千倶胝 & 経名・訳者名・固定句 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{意味・文体・典拠マーカーの三層参照源混合地図の概念図。同じ対象チャンク列を、意味層、文体・語彙層、典拠マーカー層の三つの指標で読み分け、層ごとの重み差から近さの種類のズレを解釈する。典拠マーカー層は、引用・参照関係そのものの完全検出ではなく、明示的マーカーによる探索的 proxy である。}
...
\begin{figure}[H]
  ...
  \caption{阿弥陀経二訳の意味・文体・語彙・分布差分。意味スコアは本文平均ベクトル類似度、文体・語彙スコアは文字n-gram TF-IDF 類似度、分布スコアはtop-5チャンク近傍混合率を表す。典拠マーカー層は二訳本文内ではなく、親鸞文献側の経名・訳者名・固定句により評価する。}
...
図\ref{fig:kyogyoshinsho-three-layer}は、本稿の中心的な試作結果であり、『教行信証』197チャンクについて、無量寿経、観無量寿経、羅什訳阿弥陀経、玄奘訳『稱讃淨土佛攝受經』への三層参照源混合地図を示したものである。同一の『教行信証』チャンク列に対して、意味層、文体・語彙層、典拠マーカー層が異なる参照源混合を示すことを可視化する。図中には、本文中の巻見出しにもとづき、総序、教巻、行巻、信巻、証巻、真仏土巻、化身土巻の境界を補助線として付した。巻ラベルは、本文中の見出し位置をトークン位置へ変換し、各チャンクの中心トークンが属する見出しに割り当てたものである。固定長チャンクを用いているため、境界付近のチャンクは推定であり、巻・段落による自然単位チャンク化とは異なる。

表\ref{tab:kyogyoshinsho-layer-means}に、各層の平均重みをまとめる。意味層では無量寿経0.3705、観無量寿経0.2642、羅什訳阿弥陀経0.2135、玄奘訳称讃浄土経0.1517であった。文体・語彙層では、無量寿経0.2942、観無量寿経0.2402、羅什訳阿弥陀経0.2369、玄奘訳称讃浄土経0.2288となり、意味層より均された分布を示した。典拠マーカー層では、未検出0.4958、無量寿経0.4659、観無量寿経0.0303、羅什訳阿弥陀経0.0025、玄奘訳称讃浄土経0.0054であった。

この結果は、『教行信証』が三部経を根拠とするため意味層では浄土三部経、とくに無量寿経へ近く出る一方、文体・語彙層では引用・論述・祖師文献が混ざるため参照源が均され、典拠マーカー層では辞書で拾える手がかりだけが断続的に現れることを示している。ただし、典拠マーカー層は小規模な辞書に依存するため、低い値は典拠関係の不在を意味しない。むしろ、意味層・文体・語彙層・典拠マーカー層が一致しない箇所を、精査すべき候補として浮かび上がらせるための探索図である。
...
典拠マーカー層 & 0.4958 & 0.4659 & 0.0303 & 0.0025 & 0.0054 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{『教行信証』の三層参照源混合地図。上段は埋め込み類似度に基づく意味層、中段は文字n-gram TF-IDF に基づく文体・語彙層、下段は経名・訳者名・固定句辞書に基づく典拠マーカー層を表す。各層の重みは層内での相対値であり、層間で直接比較するものではない。意味層・文体・語彙層では四参照源内で必ず重みが配分されるため、最大重みをもつ参照源が直ちに真の典拠であることを意味しない。巻区分はチャンク中心位置による推定であり、下段の「未検出」は、辞書マーカーが検出されなかったチャンクを示す。}
  \label{fig:kyogyoshinsho-three-layer}
\end{figure}

図\ref{fig:kyogyoshinsho-volume-means}と表\ref{tab:kyogyoshinsho-volume-dominants}は、同じ三層重みを巻別に平均したものである。意味層では、総序を含むすべての巻で無量寿経が最大となった。とくに教巻、証巻、真仏土巻では無量寿経への重みが相対的に高く、行巻、信巻、化身土巻では観無量寿経・阿弥陀経二訳も一定の比率で混ざる。これは、『教行信証』の巻別構成を踏まえた伝統的読解と照合すべき予備的観察であり、本稿の計算結果だけで巻ごとの教義的性格を確定するものではない。

一方、典拠マーカー層では、教巻、行巻、証巻では無量寿経系マーカーが最大となるが、信巻、真仏土巻、化身土巻では未検出が最大となった。これは、これらの巻に典拠関係がないという意味ではない。むしろ、固定句・経名・訳者名だけでは拾えない議論、釈義、引用経路が多い可能性を示し、伝統的な巻別解釈や精密な引文調査と接続するための候補箇所を示すものと読める。

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{../figures/kyogyoshinsho-volume-source-means.png}
  \caption{『教行信証』巻別の参照源傾向。各巻に属すると推定されたチャンクについて、意味層、文体・語彙層、典拠マーカー層の重みを平均した。巻区分はチャンク中心位置による推定であり、巻境界付近のチャンクは厳密な段落・章単位ではない。}
...
\centering
\small
\caption{『教行信証』巻別平均における最大参照源}
\label{tab:kyogyoshinsho-volume-dominants}
\begin{tabular}{lllll}
\toprule
巻 & チャンク範囲 & 意味層最大 & 文体・語彙層最大 & 典拠マーカー層最大 \\
\midrule
総序 & 0000--0000 & 無量寿経 0.5008 & 無量寿経 0.4899 & 無量寿経 1.0000 \\
教巻 & 0001--0002 & 無量寿経 0.5744 & 無量寿経 0.5390 & 無量寿経 0.9444 \\
行巻 & 0003--0041 & 無量寿経 0.3436 & 無量寿経 0.2927 & 無量寿経 0.6840 \\
信巻 & 0042--0095 & 無量寿経 0.3188 & 無量寿経 0.2950 & 未検出 0.4733 \\
証巻 & 0096--0111 & 無量寿経 0.4648 & 無量寿経 0.3618 & 無量寿経 0.6319 \\
真仏土巻 & 0112--0134 & 無量寿経 0.5287 & 無量寿経 0.2992 & 未検出 0.5797 \\
化身土巻 & 0135--0196 & 無量寿経 0.3408 & 無量寿経 0.2640 & 未検出 0.6720 \\
\bottomrule
\end{tabular}
\end{table}
...
結果として、Toh 106 と T0676 の組み合わせでは、84000英訳の最近傍は対応する漢訳『解深密経』となった。ただし、これは多言語対応一般の性能評価ではなく、三層地図の枠組みが多言語探索へ拡張可能であることを示す小規模パイロットである。維摩経や金剛経も0.69台で近く、大乗経典一般の主題的近さも反映されている。したがって、同一経典判定として評価するには、対応ペアと非対応ペアを増やし、top-1 accuracy、MRR、ROC-AUC などで検証する必要がある。
...
以上を踏まえると、分析単位は三層に分けるのがよい。第一に、内容・主題・教義的近接を捉える意味マップである。第二に、訳語、助字、句法、音写語、表記、文長などを捉える文体・語彙マップである。第三に、経名、訳者名、固定句、注釈引文、祖師文献引用を捉える典拠マーカーマップである。本稿は、この三層の必要性を述べるだけでなく、『教行信証』に対する三層参照源混合地図として操作化した。これにより、同じチャンク列であっても、意味層では浄土三部経への近さ、文体・語彙層では表現上の拡散、典拠マーカー層では経名・訳者名・固定句の断続性が異なる形で現れることを確認した。

親鸞文献における阿弥陀経二訳の参照分析も、同じ問題を示している。『入出二門偈』のように玄奘訳『称讃浄土経』を明示する資料では、経名・訳者名そのものが強い証拠となる。一方、『教行信証』のような総合的文献では、意味ベクトルは二訳の近接を反映するものの、引用経路や注釈伝統までは自動的に分解しない。三層参照源混合地図は、この分解不能性を失敗として扱うのではなく、層間差分として可視化する。今後の分析では、経文本文、注釈引文、祖師文献引用を分け、意味層・文体・語彙層・典拠マーカー層のズレがどの箇所で大きくなるかを精査することが重要である。
...
\noindent\textbf{三層参照源混合地図（source-mixture map）：} 三層参照源混合地図は、ある対象チャンクが複数の参照源のどれにどの程度近いかを、重みの混合として表示する図である。本稿では『教行信証』の各チャンクについて、無量寿経、観無量寿経、羅什訳阿弥陀経、玄奘訳称讃浄土経への近さを、意味層、文体・語彙層、典拠マーカー層に分けて表示した。
...
\section*{著者責任とAI支援の範囲}

本稿の著者は上山大信である。本稿は、AI支援を用いた人文系データ解析論文制作のプロトタイプでもあるため、通常の謝辞とは別に、著者責任とAI支援の範囲を明示する。近年の出版倫理指針では、AIツールや大規模言語モデルは、内容の正確性、完全性、独創性、最終承認に責任を負うことができないため、著者として扱わず、使用したツール名、用途、人間による確認責任を明記することが求められている\cite{icmje-ai,elsevier-ai,springer-ai}。また、研究への寄与を透明化するための語彙として CRediT などの貢献役割分類も広く用いられている\cite{credit-taxonomy}。

本研究において、上山大信は、研究全体の構想、研究課題の設定、対象文献の選定、仏教学的解釈の方向づけ、結果の読み方に関する判断、公開方針、最終的な内容確認を担った。CRediT 的にいえば、主として Conceptualization、Resources、Supervision、Validation、Writing--review \& editing、Project administration に相当する役割である。したがって、本稿の最終的な責任は上山大信に属する。

Codex GPT-5.5 xhigh は、上山大信の指示と確認のもとで、実験スクリプトの実装、本文取得・前処理の補助、埋め込み生成パイプライン、計算実行と解析結果の整理、可視化、HTML/PDF生成、Gitによる変更記録、草稿作成、改稿、検証手順の実行を支援した。CRediT 的な作業内容に対応させれば、Software、Data curation、Formal analysis、Visualization、Writing--original draft、Writing--review \& editing の一部に関わる支援である。ただし、Codex は著者または共同著者ではなく、研究判断および最終責任を負う主体ではない。

ChatGPT 5.5 Pro xhigh は、査読者役として、問題設定、先行研究との位置づけ、独自性の表現、限界の記述、三層地図の説明、公開向けの表現について助言を与えた。これらの助言は、Codexによる改稿作業および上山大信による判断を通じて取捨選択され、本稿に反映された。なお、AI支援によって生成・改稿された文章、図表、コード、参考文献記述については、人間が確認すべき対象であり、AI出力を一次情報源として扱わない。
...
本稿の本文および図表は Creative Commons Attribution 4.0 International (CC BY 4.0)（\url{https://creativecommons.org/licenses/by/4.0/}）で公開する。解析コードは、付属リポジトリの \texttt{LICENSE-CODE} にもとづき MIT License とする。SAT、聖教DB、84000 などから取得した元本文は本稿・公開データでは再配布せず、各提供元の利用条件に従う。
...
\newblock \url{https://www.icmje.org/recommendations/browse/artificial-intelligence/ai-use-by-authors.html}（2026年6月2日閲覧）

\bibitem{elsevier-ai}
Elsevier,
\newblock ``The use of AI and AI-assisted technologies in writing for Elsevier,''
\newblock \url{https://www.elsevier.com/about/policies-and-standards/the-use-of-generative-ai-and-ai-assisted-technologies-in-writing-for-elsevier}（2026年6月2日閲覧）

\bibitem{springer-ai}
Springer Nature,
\newblock ``Artificial Intelligence (AI),''
\newblock \url{https://link.springer.com/brands/springer/journal-policies}（2026年6月2日閲覧）

\bibitem{credit-taxonomy}
National Information Standards Organization,
\newblock ``CRediT: Contributor Role Taxonomy,''
\newblock \url{https://credit.niso.org/}（2026年6月2日閲覧）
