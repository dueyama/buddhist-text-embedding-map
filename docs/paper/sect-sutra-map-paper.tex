...
本稿では、阿弥陀経二訳および親鸞文献を読むための試験的な枠組みとして、意味層、文体・語彙層、明示マーカー層の三層を分ける（図\ref{fig:three-layer-concept}）。この三層を同じ対象チャンク列に重ねて表示する図を、本稿では三層参照源混合地図（source-mixture map）と呼ぶ。第三層は、現段階では経名・訳者名・固定句辞書による典拠マーカー層として操作化する。中心命題は、意味的に近いこと、文体・語彙的に近いこと、引用・学習経路として近いことは同じではない、という点である。
...
\begin{figure}[H]
  ...
  \caption{意味・文体・典拠マーカーの三層参照源混合地図の概念図。同じ対象チャンク列を、意味層、文体・語彙層、明示マーカー層の三つの指標で読み分け、層ごとの重み差から近さの種類のズレを解釈する。}
...
図\ref{fig:kyogyoshinsho-three-layer}は、『教行信証』197チャンクについて、無量寿経、観無量寿経、羅什訳阿弥陀経、玄奘訳『稱讃淨土佛攝受經』への三層参照源混合地図を示したものである。図中には、本文中の巻見出しにもとづき、総序、教巻、行巻、信巻、証巻、真仏土巻、化身土巻の境界を補助線として付した。巻ラベルは、本文中の見出し位置をトークン位置へ変換し、各チャンクの中心トークンが属する見出しに割り当てたものである。固定長チャンクを用いているため、境界付近のチャンクは推定であり、巻・段落による自然単位チャンク化とは異なる。
...
\begin{figure}[H]
  ...
  \caption{『教行信証』の三層参照源混合地図。上段は埋め込み類似度に基づく意味層、中段は文字n-gram TF-IDF に基づく文体・語彙層、下段は経名・訳者名・固定句辞書に基づく明示マーカー層を表す。各層の重みは層内での相対値であり、層間で直接比較するものではない。巻区分はチャンク中心位置による推定であり、下段の「未検出」は、辞書マーカーが検出されなかったチャンクを示す。}
...
一方、明示マーカー層では、教巻、行巻、証巻では無量寿経系マーカーが最大となるが、信巻、真仏土巻、化身土巻では未検出が最大となった。これは、これらの巻に典拠関係がないという意味ではない。むしろ、固定句・経名・訳者名だけでは拾えない議論、釈義、引用経路が多い可能性を示し、伝統的な巻別解釈や精密な引文調査と接続するための候補箇所を示すものと読める。

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{../figures/kyogyoshinsho-volume-source-means.png}
  \caption{『教行信証』巻別の参照源傾向。各巻に属すると推定されたチャンクについて、意味層、文体・語彙層、明示マーカー層の重みを平均した。巻区分はチャンク中心位置による推定であり、巻境界付近のチャンクは厳密な段落・章単位ではない。}
...
\begin{tabular}{lllll}
\toprule
巻 & チャンク範囲 & 意味層最大 & 文体・語彙層最大 & 明示マーカー層最大 \\
\midrule
