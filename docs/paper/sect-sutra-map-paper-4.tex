...
多言語研究においても、漢訳仏典と古典チベット語仏典の cross-linguistic semantic textual similarity が検討されており\cite{felbur2022}、近年はPāli、Sanskrit、Buddhist Chinese、Tibetan を対象とする多言語検索・機械翻訳・意味検索の基盤も展開している\cite{mitra2026}。したがって、本稿の84000英訳と漢訳『解深密経』の比較は、多言語対応の初提示ではなく、既存の多言語 Buddhist NLP の流れに接続する小規模パイロットである。

以上を踏まえると、本稿の貢献は、埋め込み手法そのものの初適用ではない。本稿の特色は、日本仏教の宗派別参照テキスト群を、分類対象ではなく探索地図として意味空間上に配置し、本文平均ベクトル、チャンク分布、チャンク近傍混合率、異訳比較、親鸞文献の引用・参照指標を一つの予備的枠組みに統合する点にある。ただし現段階では、特定の訳者判定モデルや宗派分類器を提示するものではない。中心命題は、意味埋め込みは内容・主題的近接を探索するには有効だが、宗派的伝統、訳者差、引用経路を精密に判定するには、文体特徴量および引用・参照情報を別レイヤーとして導入する必要がある、という点にある。
...
図\ref{fig:amida-comparison}の「全文直接埋め込み」値0.8943は、阿弥陀経二訳をそれぞれ一つの入力として直接埋め込んだ先行小実験の値である。一方、表\ref{tab:amida-metric-consistency}のチャンク平均ベクトル類似度0.8825は、本稿の宗派別参照群コーパスで用いたチャンク平均方式による値である。両者は前処理単位が異なるため完全には一致しないが、いずれも意味的近接が高いことを示している。
...
したがって、本研究の現段階の成果は、宗派分類器の完成ではなく、仏教文献群を探索するための意味地図の基盤を構築した点にある。既存研究における word embedding、並行句検出、cross-lingual semantic textual similarity、stylometry を踏まえると、本稿の独自性は、これらを置き換えることではなく、日本仏教の宗派別参照群、阿弥陀経二訳、親鸞文献、チャンク分布、多言語パイロットを一つの探索枠組みに接続した点にある。今後、データを拡張し、意味マップ、文体マップ、引用・参照マップを分離して重ねることで、宗派、訳者、時代、日本撰述漢文の差異をより精密に検討できると考えられる。
...
\newblock \url{https://cbeta.org/}

\bibitem{bdrc}
Buddhist Digital Resource Center。
\newblock Buddhist Digital Resource Center。
\newblock \url{https://www.bdrc.io/}
...
Dharmamitra。
\newblock DharmaNexus。
\newblock \url{https://dharmamitra.org/nexus}
