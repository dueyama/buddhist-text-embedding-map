...
| --- | --- | --- | --- | --- |
| 1 | 解深密経 | `T0676` | 84000 `Toh 106` | 既存 corpus に玄奘訳があり、84000 英訳が公開済みなので初回の多言語比較に向く。 |
| 2 | 金剛経 | `T0235` | 84000 `Toh 16` | 既存 corpus に漢訳があるが、84000 側の公開状況を確認してから使う。 |
| 3 | 維摩経 | `T0475` | Toh 番号は要確認 | 既存 corpus に漢訳があり、大乗経典として比較しやすい。 |
...
1. `experiments/multilingual_sutra_map/manifest.json` を作り、まず解深密経を登録する。
2. 漢訳 `T0676` は既存 processed text を再利用する。
3. 84000 `Toh 106` はまず英訳を対象にし、本文利用条件と credit を manifest に入れる。
